/**
 * 迷宫生成模块
 * 使用Kruskal算法生成连通迷宫，并添加小房间
 */

const MazeGenerator = {
    SIZE: 40,

    /**
     * 生成一层地牢
     */
    generateLevel(level, rng) {
        let grid = this._generateBaseMaze(rng);
        grid = this._addSmallRooms(grid, rng);

        if (level === 5) {
            grid = this._createBossHall(grid);
        }

        this._setBoundaryWalls(grid);
        return grid;
    },

    /**
     * Kruskal算法生成基础迷宫
     */
    _generateBaseMaze(rng) {
        const SIZE = this.SIZE;
        let grid = Array(SIZE).fill().map(() => Array(SIZE).fill(0));

        // 初始化所有奇行奇列为路
        for (let r = 1; r < SIZE - 1; r += 2) {
            for (let c = 1; c < SIZE - 1; c += 2) {
                grid[r][c] = 1;
            }
        }

        // 收集所有墙
        let edges = [];
        for (let r = 1; r < SIZE - 1; r += 2) {
            for (let c = 1; c < SIZE - 3; c += 2) {
                edges.push({ wallRow: r, wallCol: c + 1, a: [r, c], b: [r, c + 2] });
            }
        }
        for (let r = 1; r < SIZE - 3; r += 2) {
            for (let c = 1; c < SIZE - 1; c += 2) {
                edges.push({ wallRow: r + 1, wallCol: c, a: [r, c], b: [r + 2, c] });
            }
        }

        // 随机打乱
        RNG.shuffle(rng, edges);

        const uf = new UnionFind();

        // Kruskal算法打通墙
        for (let e of edges) {
            let ka = e.a[0] + ',' + e.a[1];
            let kb = e.b[0] + ',' + e.b[1];
            if (uf.find(ka) !== uf.find(kb)) {
                grid[e.wallRow][e.wallCol] = 1;
                uf.union(ka, kb);
            }
        }

        return grid;
    },

    /**
     * 添加小房间
     */
    _addSmallRooms(grid, rng) {
        const SIZE = this.SIZE;
        const numSmallRooms = 6 + Math.floor(rng() * 8);

        for (let i = 0; i < numSmallRooms; i++) {
            let w = 3 + Math.floor(rng() * 4);
            let h = 3 + Math.floor(rng() * 4);
            let x = 2 + Math.floor(rng() * (SIZE - w - 4));
            let y = 2 + Math.floor(rng() * (SIZE - h - 4));

            // 挖空房间区域
            for (let ry = y; ry < y + h; ry++) {
                for (let rx = x; rx < x + w; rx++) {
                    if (ry > 0 && ry < SIZE - 1 && rx > 0 && rx < SIZE - 1) {
                        grid[ry][rx] = 1;
                    }
                }
            }

            // 随机打通几个出口
            for (let tries = 0; tries < 4; tries++) {
                let doorX = x + Math.floor(rng() * w);
                let doorY = y + Math.floor(rng() * h);
                let dir = Math.floor(rng() * 4);

                if (dir === 0 && doorY > 2) grid[doorY - 1][doorX] = 1;
                if (dir === 1 && doorY < SIZE - 2) grid[doorY + 1][doorX] = 1;
                if (dir === 2 && doorX > 2) grid[doorY][doorX - 1] = 1;
                if (dir === 3 && doorX < SIZE - 2) grid[doorY][doorX + 1] = 1;
            }
        }

        return grid;
    },

    /**
     * 创建Boss大厅（第5层）
     */
    _createBossHall(grid) {
        const SIZE = this.SIZE;

        // 清空中央区域
        for (let y = 8; y < SIZE - 8; y++) {
            for (let x = 8; x < SIZE - 8; x++) {
                grid[y][x] = 1;
            }
        }

        // 添加两排柱子
        for (let y = 12; y < SIZE - 12; y += 5) {
            for (let x = 12; x < SIZE - 12; x += 5) {
                if (Math.abs(y - x) < 8 || Math.abs(y + x - SIZE) < 8) {
                    grid[y][x] = 0; // 柱子
                    if (y + 1 < SIZE - 1) grid[y + 1][x] = 0;
                    if (x + 1 < SIZE - 1) grid[y][x + 1] = 0;
                }
            }
        }

        return grid;
    },

    /**
     * 设置边界墙
     */
    _setBoundaryWalls(grid) {
        const SIZE = this.SIZE;
        for (let i = 0; i < SIZE; i++) {
            grid[0][i] = 0;
            grid[SIZE - 1][i] = 0;
            grid[i][0] = 0;
            grid[i][SIZE - 1] = 0;
        }
    },

    /**
     * 放置物品、事件和楼梯
     */
    placeItemsAndEvents(grid, level, rng) {
        let newGrid = grid.map(row => [...row]);
        let free = this._getFreeCells(newGrid);
        RNG.shuffle(rng, free);

        // 放置道具（剑、盾、血药）
        const itemCount = 3 + level + Math.floor(rng() * 4);
        for (let i = 0; i < itemCount && i < free.length; i++) {
            let type = Math.floor(rng() * 3) + 2; // 2,3,4
            newGrid[free[i][0]][free[i][1]] = type;
        }

        // 放置随机事件
        const eventCount = 2 + Math.floor(rng() * 4);
        for (let i = 0; i < eventCount && i + itemCount < free.length; i++) {
            let [r, c] = free[i + itemCount];
            newGrid[r][c] = 6; // 事件
        }

        return newGrid;
    },

    /**
     * 放置楼梯
     */
    placeStairs(grid, rng) {
        let newGrid = grid.map(row => [...row]);
        let free = this._getFreeCells(newGrid);
        free = free.filter(([r, c]) => !(r === 1 && c === 1));
        RNG.shuffle(rng, free);

        if (free.length > 0) {
            let [r, c] = free[0];
            newGrid[r][c] = 7; // 楼梯
            return { grid: newGrid, stairsPos: { row: r, col: c } };
        }

        return { grid: newGrid, stairsPos: { row: 38, col: 38 } };
    },

    /**
     * 获取所有可走的路
     */
    _getFreeCells(grid) {
        const SIZE = this.SIZE;
        let free = [];
        for (let r = 1; r < SIZE - 1; r++) {
            for (let c = 1; c < SIZE - 1; c++) {
                if (grid[r][c] === 1) {
                    free.push([r, c]);
                }
            }
        }
        return free;
    }
};
